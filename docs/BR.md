...
This section specifies the technical profile requirements for all Certificates and Precertificates issued under these Requirements:
...
In accordance with [Section 7.1](#71-certificate-profile), a CA’s CP/CPS MUST define its specific issuance practices for each Certificate Profile it issues. Each CA-defined Certificate Profile in Section 7.1 of the CP/CPS operates as a strict allowlist ("Default Deny"): the CA MUST explicitly specify which optional fields, extensions, Distinguished Name attributes, `GeneralName` types, `AccessDescription` methods, `PolicyInformation` identifiers/qualifiers, `KeyUsage` bits, and `ExtendedKeyUsage` purposes permitted by the applicable baseline profile table are included in certificates issued under that profile. Any element not explicitly permitted by the CA’s CP/CPS profile MUST NOT be included in issued certificates.

**Transition Note:** Prior to $DATE_1, CAs MAY issue Certificates conforming either to the Certificate Profiles specified in this section or to the applicable Certificate Profiles specified in Version 2.3.0 of these Requirements. Effective $DATE_1, all newly issued Certificates and Precertificates MUST conform to the Certificate Profiles specified in this section.
...
| **`tbsCertificate.signature`** | — | **MUST** | — | Permitted CA signature algorithm ([Section 7.1.3.2](#7132-signature-algorithmidentifier)). MUST match outer `signatureAlgorithm` byte-for-byte. |
...
| **`tbsCertificate.subject`** | — | **MUST** | — | Encoded per [Section 7.1.4](#714-name-forms). Each RDN MUST contain a single `AttributeTypeAndValue`. Attributes MUST NOT contain metadata-only values (`"."`, `"-"`, `" "`). |
|   ↳ **`subject:countryName`** | `2.5.4.6` | **MUST** | — | Two-letter ISO 3166-1 alpha-2 country code for the country in which the CA place of business is located ([Section 3.2.2.3](#3223-verification-of-country)). |
|   ↳ **`subject:stateOrProvinceName`** | `2.5.4.8` | **MAY** | — | Verified CA state or province name ([Section 3.2.2.1](#3221-identity)). |
|   ↳ **`subject:localityName`** | `2.5.4.7` | **MAY** | — | Verified CA locality name ([Section 3.2.2.1](#3221-identity)). |
|   ↳ **`subject:postalCode`** | `2.5.4.17` | **MAY** | — | Verified CA postal/ZIP code ([Section 3.2.2.1](#3221-identity)). |
|   ↳ **`subject:streetAddress`** | `2.5.4.9` | **MAY** | — | Verified CA street address ([Section 3.2.2.1](#3221-identity)). |
|   ↳ **`subject:organizationName`** | `2.5.4.10` | **MUST** | — | Verified legal name or DBA of the CA ([Section 3.2.2.2](#3222-dbatradename)). |
|   ↳ **`subject:organizationalUnitName`** | `2.5.4.11` | **MUST NOT** | — | MUST NOT be present in Root CA Certificates. |
|   ↳ **`subject:commonName`** | `2.5.4.3` | **MUST** | — | Unique identifier for the Root CA Certificate. |
|   ↳ **`subject:<any other attribute>`** | — | **NOT RECOMMENDED** | — | See [Section 7.1.4.4](#7144-other-subject-attributes). |
| **`tbsCertificate.subjectPublicKeyInfo`** | — | **MUST** | — | **RSA (`1.2.840.113549.1.1.1`):** Modulus size ≥ 2048 bits and evenly divisible by 8 ([Section 6.1.5](#615-key-sizes)); odd public exponent ≥ 3 (SHOULD be between 2¹⁶ + 1 and 2²⁵⁶ − 1; [Section 6.1.6](#616-public-key-parameters-generation-and-quality-checking)); explicit `NULL` parameters (`300d06092a864886f70d0101010500`; [Section 7.1.3.1.1](#71311-rsa)).<br>**ECDSA (`1.2.840.10045.2.1`):** `namedCurve` P-256 (`1.2.840.10045.3.1.7`), P-384 (`1.3.132.0.34`), or P-521 (`1.3.132.0.35`), uncompressed point format (`0x04`; [Section 7.1.3.1.2](#71312-ecdsa)). |
...
| **`authorityKeyIdentifier` (AKI)** | `2.5.29.35` | **RECOMMENDED** | **N** | If present, `keyIdentifier` MUST be identical to the `subjectKeyIdentifier` field. `authorityCertIssuer` and `authorityCertSerialNumber` MUST NOT be present. |
...
| **`subjectKeyIdentifier` (SKI)** | `2.5.29.14` | **MUST** | **N** | Unique key identifier generated per [RFC 5280 Section 4.2.1.2](https://datatracker.ietf.org/doc/html/rfc5280#section-4.2.1.2). |
| **`extKeyUsage` (EKU)** | `2.5.29.37` | **MUST NOT** | — | MUST NOT be present in Root CA Certificates. |
| **`certificatePolicies`** | `2.5.29.32` | **NOT RECOMMENDED** | **N** | See [Section 7.1.6](#716-certificate-policy-object-identifier). |
| **`cRLDistributionPoints`** | `2.5.29.31` | **SHOULD NOT** | **N** | SHOULD NOT be present in Root CA Certificates. |
| **Signed Certificate Timestamp List** | `1.3.6.1.4.1.11129.2.4.2` | **MAY** | **N** | `OCTET STRING` containing `SignedCertificateTimestampList` per [RFC 6962 Section 3.3](https://datatracker.ietf.org/doc/html/rfc6962#section-3.3). |
| **Any other extension** | — | **NOT RECOMMENDED** | — | MUST NOT be marked critical unless explicitly permitted by these Requirements. |
...
| **`tbsCertificate.serialNumber`** | — | **MUST** | — | Positive non-sequential integer > 0 and < 2¹⁵⁹ containing ≥ 64 bits of CSPRNG output. |
| **`tbsCertificate.signature`** | — | **MUST** | — | Permitted CA signature algorithm ([Section 7.1.3.2](#7132-signature-algorithmidentifier)). MUST match outer `signatureAlgorithm` byte-for-byte. |
| **`tbsCertificate.issuer`** | — | **MUST** | — | MUST be byte-for-byte identical to the `subject` field of the Issuing CA ([Section 7.1.4.1](#7141-name-encoding)). |
| **`tbsCertificate.validity`** | — | **MUST** | — | **`notBefore`:** Between 1 day prior to signing and the time of signing.<br>**`notAfter`:** Greater than the time of signing; within the Issuing CA validity period. |
| **`tbsCertificate.subject`** | — | **MUST** | — | Encoded per [Section 7.1.4](#714-name-forms). Each RDN MUST contain a single `AttributeTypeAndValue`. Attributes MUST NOT contain metadata-only values (`"."`, `"-"`, `" "`). |
|   ↳ **`subject:countryName`** | `2.5.4.6` | **MUST** | — | Two-letter ISO 3166-1 alpha-2 country code for the country in which the CA place of business is located ([Section 3.2.2.3](#3223-verification-of-country)). |
|   ↳ **`subject:stateOrProvinceName`** | `2.5.4.8` | **MAY** | — | Verified CA state or province name ([Section 3.2.2.1](#3221-identity)). |
|   ↳ **`subject:localityName`** | `2.5.4.7` | **MAY** | — | Verified CA locality name ([Section 3.2.2.1](#3221-identity)). |
...
|   ↳ **`subject:commonName`** | `2.5.4.3` | **MUST** | — | Unique identifier for the Subordinate CA Certificate. |
|   ↳ **`subject:<any other attribute>`** | — | **NOT RECOMMENDED** | — | See [Section 7.1.4.4](#7144-other-subject-attributes). |
| **`tbsCertificate.subjectPublicKeyInfo`** | — | **MUST** | — | **RSA (`1.2.840.113549.1.1.1`):** Modulus size ≥ 2048 bits and evenly divisible by 8 ([Section 6.1.5](#615-key-sizes)); odd public exponent ≥ 3 (SHOULD be between 2¹⁶ + 1 and 2²⁵⁶ − 1; [Section 6.1.6](#616-public-key-parameters-generation-and-quality-checking)); explicit `NULL` parameters (`300d06092a864886f70d0101010500`; [Section 7.1.3.1.1](#71311-rsa)).<br>**ECDSA (`1.2.840.10045.2.1`):** `namedCurve` P-256 (`1.2.840.10045.3.1.7`), P-384 (`1.3.132.0.34`), or P-521 (`1.3.132.0.35`), uncompressed point format (`0x04`; [Section 7.1.3.1.2](#71312-ecdsa)). |
| **`tbsCertificate.issuerUniqueID`** | — | **MUST NOT** | — | MUST NOT be present. |
| **`tbsCertificate.subjectUniqueID`** | — | **MUST NOT** | — | MUST NOT be present. |
...
|   ↳ **`policyIdentifier` (Affiliated CA)** | `2.5.29.32.0` | **MAY** | — | `anyPolicy` (`2.5.29.32.0`) MAY be used ONLY if the Subordinate CA is an Affiliate of the Issuing CA and expresses no policy restrictions; if present, it MUST be the only `PolicyInformation` value. |
|   ↳ **`policyIdentifier` (Policy Restricted)** | `2.23.140.1.2.1` / `2.23.140.1.2.2` / `2.23.140.1.1` | **MUST** | — | If not Affiliated `anyPolicy`, MUST include at least one Reserved Certificate Policy Identifier (`2.23.140.1.2.1` DV, `2.23.140.1.2.2` OV, or `2.23.140.1.1` EV) associated with the Subscriber Certificates issued directly or transitively by this Subordinate CA. If Subscriber Certificates chain directly to this Subordinate CA, it MUST contain **exactly one** Reserved Certificate Policy Identifier. `anyPolicy` MUST NOT be present. |
|   ↳ **`policyIdentifier` (CA-defined)** | — | **MAY** | — | Additional OIDs documented in Section 7.1 of the CA CP/CPS. |
|   ↳ **`policyQualifiers` (`id-qt-cps`)** | `1.3.6.1.5.5.7.2.1` | **NOT RECOMMENDED** | — | If present, MUST contain ONLY `id-qt-cps` (`IA5String` HTTP/HTTPS URL of CP/CPS). Any other qualifier **MUST NOT** be present. |
| **`cRLDistributionPoints`** | `2.5.29.31` | **MUST** | **N** | MUST contain ≥ 1 `DistributionPoint` with `fullName` containing an `http://` `uniformResourceIdentifier` for the Issuing CA CRL service. `reasons` and `cRLIssuer` MUST NOT be present. |
| **`keyUsage`** | `2.5.29.15` | **MUST** | **Y** | Bit string asserting permitted key usages: |
|   ↳ **`keyCertSign`** | — | **MUST** | — | MUST be asserted. |
|   ↳ **`cRLSign`** | — | **MUST** | — | MUST be asserted. |
...
|   ↳ **All other Key Usage bits** | — | **MUST NOT** | — | `nonRepudiation`, `keyEncipherment`, `dataEncipherment`, `keyAgreement`, `encipherOnly`, and `decipherOnly` MUST NOT be asserted. |
| **`subjectKeyIdentifier` (SKI)** | `2.5.29.14` | **MUST** | **N** | Unique key identifier generated per [RFC 5280 Section 4.2.1.2](https://datatracker.ietf.org/doc/html/rfc5280#section-4.2.1.2). |
| **`extKeyUsage` (EKU)** | `2.5.29.37` | **MUST** | **N** | `SEQUENCE` of `KeyPurposeId` OIDs: |
|   ↳ **`id-kp-serverAuth`** | `1.3.6.1.5.5.7.3.1` | **MUST** | — | MUST be present (`1.3.6.1.5.5.7.3.1`). |
|   ↳ **`<any other KeyPurposeId>`** | — | **MUST NOT** | — | **MUST NOT** contain `id-kp-clientAuth`, `anyExtendedKeyUsage`, or any other `KeyPurposeId` OID. |
| **`authorityInformationAccess` (AIA)** | `1.3.6.1.5.5.7.1.1` | **SHOULD** | **N** | `SEQUENCE` of `AccessDescription` entries: |
|   ↳ **`id-ad-ocsp`** | `1.3.6.1.5.5.7.48.1` | **MAY** | — | `uniformResourceIdentifier` (`http://` OCSP responder URL). |
|   ↳ **`id-ad-caIssuers`** | `1.3.6.1.5.5.7.48.2` | **MAY** | — | `uniformResourceIdentifier` (`http://` URL of Issuing CA certificate). |
|   ↳ **`<any other accessMethod>`** | — | **MUST NOT** | — | No other `accessMethod` OIDs permitted. |
| **`nameConstraints`** | `2.5.29.30` | **MAY** | **SHOULD** (`Y`) / **MAY** (`N`) | If present, MUST be encoded with `permittedSubtrees` / `excludedSubtrees` `GeneralSubtree` entries (`dNSName`, `iPAddress`, `directoryName`) with `minimum` and `maximum` omitted. **Note:** Presence of `nameConstraints` does not exempt the Subordinate CA from annual independent third-party WebTrust/ETSI audits ([Section 8.1](#81-frequency-or-circumstances-of-assessment)), CAA record checking ([Section 3.2.2.8](#3228-caa-records)), or OCSP unassigned serial number rules ([Section 4.9.9](#499-on-line-revocationstatus-checking-availability)). |
| **Signed Certificate Timestamp List** | `1.3.6.1.4.1.11129.2.4.2` | **MUST** | **N** | `OCTET STRING` containing `SignedCertificateTimestampList` per [RFC 6962 Section 3.3](https://datatracker.ietf.org/doc/html/rfc6962#section-3.3). |
| **Any other extension** | — | **NOT RECOMMENDED** | — | MUST NOT be marked critical unless explicitly permitted by these Requirements. |
| **`signatureAlgorithm`** | — | **MUST** | — | Outer `AlgorithmIdentifier`. MUST be byte-for-byte identical to `tbsCertificate.signature`. |
| **`signature`** | — | **MUST** | — | Cryptographic signature bit string over `tbsCertificate`. |
...
|   ↳ **`<any other KeyPurposeId>`** | — | **MUST NOT** | — | **MUST NOT** contain `anyExtendedKeyUsage` or any other `KeyPurposeId` OID. |
| **`authorityInformationAccess` (AIA)** | `1.3.6.1.5.5.7.1.1` | **SHOULD** | **N** | `SEQUENCE` of `AccessDescription` entries: |
|   ↳ **`id-ad-ocsp`** | `1.3.6.1.5.5.7.48.1` | **MAY** | — | `uniformResourceIdentifier` (`http://` OCSP responder URL). |
|   ↳ **`id-ad-caIssuers`** | `1.3.6.1.5.5.7.48.2` | **MAY** | — | `uniformResourceIdentifier` (`http://` URL of Issuing CA certificate). |
|   ↳ **`<any other accessMethod>`** | — | **MUST NOT** | — | No other `accessMethod` OIDs permitted. |
...
| **`tbsCertificate.signature`** | — | **MUST** | — | `AlgorithmIdentifier` permitted in [Section 7.1.3.2](#7132-signature-algorithmidentifier). MUST be byte-for-byte identical to the outer `signatureAlgorithm` field. |
...
| **`tbsCertificate.validity.notBefore`** | — | **MUST** | — | Encoded as `UTCTime` (dates through 2049) or `GeneralizedTime` (dates 2050+). Value MUST be no earlier than 48 hours prior to signing and no later than the time of signing. |
| **`tbsCertificate.validity.notAfter`** | — | **MUST** | — | Encoded as `UTCTime` (dates through 2049) or `GeneralizedTime` (dates 2050+). Certificate validity period MUST NOT exceed the maximum Subscriber Certificate validity period specified in [Section 6.3.2](#632-certificate-operational-periods-and-key-pair-usage-periods). |
| **`tbsCertificate.subject`** | — | **MUST** | — | `SEQUENCE` of `RelativeDistinguishedName`s encoded per [Section 7.1.4](#714-name-forms). Only the attributes listed below are permitted: |
...
|   ↳ **`id-ad-caIssuers`** | `1.3.6.1.5.5.7.48.2` | **RECOMMENDED** | — | `uniformResourceIdentifier` (`IA5String`) HTTP URL of the Issuing CA certificate. |
|   ↳ **`<any other accessMethod>`** | — | **MUST NOT** | — | MUST NOT be present. |
| **`authorityKeyIdentifier`** | `2.5.29.35` | **MUST** | **N** | `keyIdentifier` MUST be present and identical to the Issuing CA’s `subjectKeyIdentifier`. `authorityCertIssuer` and `authorityCertSerialNumber` MUST NOT be present. |
| **`basicConstraints`** | `2.5.29.19` | **MUST** | **Y / N** | `cA` MUST be `FALSE` (omitted in DER encoding). `pathLenConstraint` MUST NOT be present. |
| **`certificatePolicies`** | `2.5.29.32` | **MUST** | **N** | `SEQUENCE` of `PolicyInformation` entries governed by the rules below: |
|   ↳ **Reserved Policy OID** | `2.23.140.1.2.1` | **MUST** | — | MUST be present. RECOMMENDED as the first `PolicyInformation` entry. |
...
|   ↳ **`policyQualifiers` (`id-qt-cps`)** | `1.3.6.1.5.5.7.2.1` | **NOT RECOMMENDED** | — | `IA5String` HTTP/HTTPS URL of the CA’s CP/CPS. Any other qualifier type MUST NOT be present. |
| **`cRLDistributionPoints`** | `2.5.29.31` | **MAY** | **N** | If present, `distributionPoint.fullName` MUST contain an HTTP `uniformResourceIdentifier` (`IA5String`). `reasons` and `cRLIssuer` MUST NOT be present. |
| **`extKeyUsage`** | `2.5.29.37` | **MUST** | **N** | `SEQUENCE` of `KeyPurposeId` OIDs governed by the rules below: |
...
| **`keyUsage`** | `2.5.29.15` | **MUST** | **Y** | `BIT STRING` governed by the public key algorithm rules below: |
|   ↳ **RSA Public Keys** | — | **Conditional** | — | `digitalSignature` (**SHOULD**), `keyEncipherment` (**MAY**), `dataEncipherment` (**NOT RECOMMENDED**). All other bits **MUST NOT** be asserted. |
|   ↳ **ECC Public Keys** | — | **Conditional** | — | `digitalSignature` (**MUST**), `keyAgreement` (**NOT RECOMMENDED**). All other bits **MUST NOT** be asserted. |
| **`subjectAltName`** | `2.5.29.17` | **MUST** | **Y (if `subject` empty) / N** | MUST contain at least one `dNSName` or `iPAddress` `GeneralName`: |
|   ↳ **`dNSName`** | `[2]` | **MAY** | — | Validated FQDN or Wildcard per [Section 3.2.2.4](#3224-validation-of-domain-authorization-or-control) and [Section 3.2.2.6](#3226-wildcard-domain-validation). MUST NOT contain an Internal Name or Reverse Zone Suffix; MUST use LDH/P-Labels without trailing dot. |
|   ↳ **`iPAddress`** | `[7]` | **MAY** | — | Validated IPv4 or IPv6 address per [Section 3.2.2.5](#3225-authentication-for-an-ip-address). MUST NOT contain a Reserved IP Address. |
|   ↳ **`<any other GeneralName>`** | — | **MUST NOT** | — | MUST NOT be present. |
| **`subjectKeyIdentifier`** | `2.5.29.14` | **NOT RECOMMENDED** | **N** | If present, derived from `subjectPublicKeyInfo` per [RFC 5280 Section 4.2.1.2](https://datatracker.ietf.org/doc/html/rfc5280#section-4.2.1.2). |
| **Precertificate Poison** | `1.3.6.1.4.1.11129.2.4.3` | **Conditional** | **Y** | **Precertificate:** MUST be present, marked critical (`Y`), encoded as ASN.1 `NULL` (`05 00`).<br>**Final Certificate:** MUST NOT be present. |
| **Signed Certificate Timestamp List** | `1.3.6.1.4.1.11129.2.4.2` | **Conditional** | **N** | **Precertificate:** MUST NOT be present.<br>**Final Certificate:** MAY be present, non-critical (`N`), formatted per [RFC 6962 Section 3.3](https://datatracker.ietf.org/doc/html/rfc6962#section-3.3). |
| **Any other extension** | — | **NOT RECOMMENDED** | **N** | MUST NOT be marked critical unless explicitly permitted by these Requirements. |
...
| **`tbsCertificate.validity.notBefore`** | — | **MUST** | — | Encoded as `UTCTime` (dates through 2049) or `GeneralizedTime` (dates 2050+). Value MUST be no earlier than 48 hours prior to signing and no later than the time of signing. |
| **`tbsCertificate.validity.notAfter`** | — | **MUST** | — | Encoded as `UTCTime` (dates through 2049) or `GeneralizedTime` (dates 2050+). Certificate validity period MUST NOT exceed the maximum Subscriber Certificate validity period specified in [Section 6.3.2](#632-certificate-operational-periods-and-key-pair-usage-periods). |
| **`tbsCertificate.subject`** | — | **MUST** | — | Non-empty `SEQUENCE` of `RelativeDistinguishedName`s encoded per [Section 7.1.4](#714-name-forms). Attributes MUST NOT contain metadata-only values (`.`, `-`, spaces). Permitted attributes: |
|   ↳ **`subject:domainComponent`** | `0.9.2342.19200300.100.1.25` | **MAY** | — | `IA5String` (max 63 chars). Ordered sequence of Domain Labels (closest to root first) verified per [Section 3.2](#32-initial-identity-validation). |
|   ↳ **`subject:countryName`** | `2.5.4.6` | **MUST** | — | `PrintableString` (2 chars). Two-letter ISO 3166-1 alpha-2 country code (or `XX`) verified per [Section 3.2.2.1](#3221-identity). |
|   ↳ **`subject:stateOrProvinceName`** | `2.5.4.8` | **MUST / MAY** | — | `UTF8String` or `PrintableString` (max 128 chars). MUST be present if `localityName` is absent; MAY be present otherwise. Verified per [Section 3.2.2.1](#3221-identity). |
|   ↳ **`subject:localityName`** | `2.5.4.7` | **MUST / MAY** | — | `UTF8String` or `PrintableString` (max 128 chars). MUST be present if `stateOrProvinceName` is absent; MAY be present otherwise. Verified per [Section 3.2.2.1](#3221-identity). |
|   ↳ **`subject:postalCode`** | `2.5.4.17` | **NOT RECOMMENDED** | — | `UTF8String` or `PrintableString` (max 40 chars). Subject’s postal code verified per [Section 3.2.2.1](#3221-identity). |
|   ↳ **`subject:streetAddress`** | `2.5.4.9` | **NOT RECOMMENDED** | — | `UTF8String` or `PrintableString` (max 128 chars). Subject’s street address verified per [Section 3.2.2.1](#3221-identity). |
|   ↳ **`subject:organizationName`** | `2.5.4.10` | **MUST** | — | `UTF8String` or `PrintableString` (max 64 chars). Subject’s legal name and/or DBA/tradename verified per [Section 3.2.2.1](#3221-identity) and [Section 3.2.2.2](#3222-dbatradename). |
...
| **`basicConstraints`** | `2.5.29.19` | **MUST** | **Y / N** | `cA` MUST be `FALSE` (omitted in DER encoding). `pathLenConstraint` MUST NOT be present. |
| **`certificatePolicies`** | `2.5.29.32` | **MUST** | **N** | `SEQUENCE` of `PolicyInformation` entries governed by the rules below: |
|   ↳ **Reserved Policy OID** | `2.23.140.1.2.2` | **MUST** | — | MUST be present. RECOMMENDED as the first `PolicyInformation` entry. |
|   ↳ **`anyPolicy`** | `2.5.29.32.0` | **MUST NOT** | — | MUST NOT be present. |
|   ↳ **CA-Defined Policy OIDs** | — | **MAY** | — | Permitted if defined and documented in the CA’s CP/CPS. |
|   ↳ **`policyQualifiers` (`id-qt-cps`)** | `1.3.6.1.5.5.7.2.1` | **NOT RECOMMENDED** | — | `IA5String` HTTP/HTTPS URL of the CA’s CP/CPS. Any other qualifier type MUST NOT be present. |
| **`cRLDistributionPoints`** | `2.5.29.31` | **MAY** | **N** | If present, `distributionPoint.fullName` MUST contain an HTTP `uniformResourceIdentifier` (`IA5String`). `reasons` and `cRLIssuer` MUST NOT be present. |
...
For a Subscriber Certificate or Precertificate to be Extended Validation (EV), it MUST conform to the following baseline profile table:
...
| **`tbsCertificate.issuer`** | — | **MUST** | — | Encoded value MUST be byte-for-byte identical to the encoded `subject` of the Issuing CA certificate. |
| **`tbsCertificate.validity.notBefore`** | — | **MUST** | — | Encoded as `UTCTime` (dates through 2049) or `GeneralizedTime` (dates 2050+). Value MUST be no earlier than 48 hours prior to signing and no later than the time of signing. |
| **`tbsCertificate.validity.notAfter`** | — | **MUST** | — | Encoded as `UTCTime` (dates through 2049) or `GeneralizedTime` (dates 2050+). Certificate validity period MUST NOT exceed the maximum EV Subscriber Certificate validity period specified in [Section 6.3.2](#632-certificate-operational-periods-and-key-pair-usage-periods) and the EV Guidelines. |
| **`tbsCertificate.subject`** | — | **MUST** | — | Non-empty `SEQUENCE` of `RelativeDistinguishedName`s encoded per [Section 7.1.4](#714-name-forms). Attributes MUST NOT contain metadata-only values (`.`, `-`, spaces). Permitted attributes: |
|   ↳ **`subject:domainComponent`** | `0.9.2342.19200300.100.1.25` | **MAY** | — | `IA5String` (max 63 chars). Ordered sequence of Domain Labels (closest to root first) verified per EV Guidelines Section 9.2.2. |
|   ↳ **`subject:countryName`** | `2.5.4.6` | **MUST** | — | `PrintableString` (2 chars). Two-letter ISO 3166-1 alpha-2 country code verified per EV Guidelines Section 9.2.5. |
|   ↳ **`subject:stateOrProvinceName`** | `2.5.4.8` | **MUST / MAY** | — | `UTF8String` or `PrintableString` (max 128 chars). MUST be present if required by EV Guidelines Section 9.2.5. |
|   ↳ **`subject:localityName`** | `2.5.4.7` | **MUST / MAY** | — | `UTF8String` or `PrintableString` (max 128 chars). MUST be present if required by EV Guidelines Section 9.2.5. |
|   ↳ **`subject:postalCode`** | `2.5.4.17` | **NOT RECOMMENDED** | — | `UTF8String` or `PrintableString` (max 40 chars). Verified per EV Guidelines Section 9.2.5. |
|   ↳ **`subject:streetAddress`** | `2.5.4.9` | **NOT RECOMMENDED** | — | `UTF8String` or `PrintableString` (max 128 chars). Verified per EV Guidelines Section 9.2.5. |
|   ↳ **`subject:organizationName`** | `2.5.4.10` | **MUST** | — | `UTF8String` or `PrintableString` (max 64 chars). Subject’s full legal organization name verified per EV Guidelines Section 9.2.1. |
|   ↳ **`subject:organizationalUnitName`** | `2.5.4.11` | **MUST NOT** | — | MUST NOT be present. |
|   ↳ **`subject:businessCategory`** | `2.5.4.15` | **MUST** | — | `UTF8String` or `PrintableString` (max 128 chars). MUST be one of `"Private Organization"`, `"Government Entity"`, `"Business Entity"`, or `"Non-Commercial Entity"` per EV Guidelines Section 9.2.3. |
|   ↳ **`subject:jurisdictionCountry`** | `1.3.6.1.4.1.311.60.2.1.3` | **MUST** | — | `PrintableString` (2 chars). Country of incorporation/registration per EV Guidelines Section 9.2.4. |
|   ↳ **`subject:jurisdictionStateOrProvince`** | `1.3.6.1.4.1.311.60.2.1.2` | **MUST / MAY** | — | `UTF8String` or `PrintableString` (max 128 chars). State/province of incorporation/registration (if applicable) per EV Guidelines Section 9.2.4. |
|   ↳ **`subject:jurisdictionLocality`** | `1.3.6.1.4.1.311.60.2.1.1` | **MUST / MAY** | — | `UTF8String` or `PrintableString` (max 128 chars). Locality of incorporation/registration (if applicable) per EV Guidelines Section 9.2.4. |
|   ↳ **`subject:serialNumber`** | `2.5.4.5` | **MUST** | — | `PrintableString` (max 64 chars). Registration number assigned by the Incorporating or Registration Agency per EV Guidelines Section 9.2.4. |
...
| **`basicConstraints`** | `2.5.29.19` | **MUST** | **Y / N** | `cA` MUST be `FALSE` (omitted in DER encoding). `pathLenConstraint` MUST NOT be present. |
| **`certificatePolicies`** | `2.5.29.32` | **MUST** | **N** | `SEQUENCE` of `PolicyInformation` entries governed by the rules below: |
|   ↳ **Reserved Policy OID** | `2.23.140.1.1` | **MUST** | — | MUST be present. RECOMMENDED as the first `PolicyInformation` entry. |
|   ↳ **`anyPolicy`** | `2.5.29.32.0` | **MUST NOT** | — | MUST NOT be present. |
|   ↳ **CA-Defined Policy OIDs** | — | **MAY** | — | Permitted if defined and documented in the CA’s CP/CPS. |
|   ↳ **`policyQualifiers` (`id-qt-cps`)** | `1.3.6.1.5.5.7.2.1` | **NOT RECOMMENDED** | — | `IA5String` HTTP/HTTPS URL of the CA’s CP/CPS. Any other qualifier type MUST NOT be present. |
| **`cRLDistributionPoints`** | `2.5.29.31` | **MAY** | **N** | If present, `distributionPoint.fullName` MUST contain an HTTP `uniformResourceIdentifier` (`IA5String`). `reasons` and `cRLIssuer` MUST NOT be present. |
...
|   ↳ **ECC Public Keys** | — | **Conditional** | — | `digitalSignature` (**MUST**), `keyAgreement` (**NOT RECOMMENDED**). All other bits **MUST NOT** be asserted. |
| **`subjectAltName`** | `2.5.29.17` | **MUST** | **N** | MUST contain at least one `dNSName` or `iPAddress` `GeneralName`: |
|   ↳ **`dNSName`** | `[2]` | **MAY** | — | Validated FQDN per [Section 3.2.2.4](#3224-validation-of-domain-authorization-or-control) and EV Guidelines Section 9.2.2. Wildcard Domain Names (`*`) MUST NOT be present except for `.onion` version 3 domain names per EV Guidelines Section 9.8.2. MUST NOT contain an Internal Name or Reverse Zone Suffix; MUST use LDH/P-Labels without trailing dot. |
|   ↳ **`iPAddress`** | `[7]` | **MAY** | — | Validated IPv4 or IPv6 address per [Section 3.2.2.5](#3225-authentication-for-an-ip-address) (where permitted by EV Guidelines). MUST NOT contain a Reserved IP Address. |
|   ↳ **`<any other GeneralName>`** | — | **MUST NOT** | — | MUST NOT be present. |
| **`subjectKeyIdentifier`** | `2.5.29.14` | **NOT RECOMMENDED** | **N** | If present, derived from `subjectPublicKeyInfo` per [RFC 5280 Section 4.2.1.2](https://datatracker.ietf.org/doc/html/rfc5280#section-4.2.1.2). |
| **Precertificate Poison** | `1.3.6.1.4.1.11129.2.4.3` | **Conditional** | **Y** | **Precertificate:** MUST be present, marked critical (`Y`), encoded as ASN.1 `NULL` (`05 00`).<br>**Final Certificate:** MUST NOT be present. |
| **Signed Certificate Timestamp List** | `1.3.6.1.4.1.11129.2.4.2` | **Conditional** | **N** | **Precertificate:** MUST NOT be present.<br>**Final Certificate:** MAY be present, non-critical (`N`), formatted per [RFC 6962 Section 3.3](https://datatracker.ietf.org/doc/html/rfc6962#section-3.3). |
...
|   ↳ **`subject:streetAddress`** | `2.5.4.9` | **MAY** | — | Verified CA street address ([Section 3.2.2.1](#3221-identity)). |
|   ↳ **`subject:organizationName`** | `2.5.4.10` | **MUST** | — | Verified legal name or DBA of the CA ([Section 3.2.2.2](#3222-dbatradename)). |
|   ↳ **`subject:organizationalUnitName`** | `2.5.4.11` | **SHOULD NOT** | — | SHOULD NOT be present. |
|   ↳ **`subject:commonName`** | `2.5.4.3` | **MUST** | — | Unique identifier for the OCSP Responder Certificate. |
|   ↳ **`subject:<any other attribute>`** | — | **NOT RECOMMENDED** | — | See [Section 7.1.4.4](#7144-other-subject-attributes). |
| **`tbsCertificate.subjectPublicKeyInfo`** | — | **MUST** | — | Encoded per [Section 7.1.3.1](#7131-subjectpublickeyinfo) and [Section 6.1.5](#615-key-sizes): **RSA (`1.2.840.113549.1.1.1`):** modulus ≥ 2048 bits evenly divisible by 8, odd public exponent ≥ 3 (SHOULD be 2¹⁶ + 1 to 2²⁵⁶ − 1), explicit `NULL` parameters; or **ECDSA (`1.2.840.10045.2.1`):** `namedCurve` NIST `P-256`, `P-384`, or `P-521`, uncompressed point format (`0x04`). |
| **`tbsCertificate.issuerUniqueID`** | — | **MUST NOT** | — | MUST NOT be present. |
| **`tbsCertificate.subjectUniqueID`** | — | **MUST NOT** | — | MUST NOT be present. |
...
| **`extKeyUsage` (EKU)** | `2.5.29.37` | **MUST** | **N** | MUST contain `id-kp-OCSPSigning` (`1.3.6.1.5.5.7.3.9`). MUST NOT contain any other `KeyPurposeId`. |
| **`id-pkix-ocsp-nocheck`** | `1.3.6.1.5.5.7.48.1.5` | **MUST** | **N** | Encoded as ASN.1 `NULL` (`05 00`). |
| **`subjectKeyIdentifier` (SKI)** | `2.5.29.14` | **MUST** | **N** | Unique key identifier generated per [RFC 5280 Section 4.2.1.2](https://datatracker.ietf.org/doc/html/rfc5280#section-4.2.1.2). |
| **`cRLDistributionPoints`** | `2.5.29.31` | **MUST NOT** | — | MUST NOT be present in OCSP Responder Certificates. |
| **`authorityInformationAccess` (AIA)** | `1.3.6.1.5.5.7.1.1` | **SHOULD NOT** | **N** | SHOULD NOT be present. |
| **Any other extension** | — | **NOT RECOMMENDED** | **N** | MUST NOT be marked critical unless explicitly permitted by these Requirements. |
| **`signatureAlgorithm`** | — | **MUST** | — | Outer `AlgorithmIdentifier`. MUST be byte-for-byte identical to `tbsCertificate.signature`. |
| **`signature`** | — | **MUST** | — | Digital signature computed over the ASN.1 DER encoded `tbsCertificate`. |
